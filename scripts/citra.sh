...
rm -r ./data-backup/citra
mkdir -p ./data-backup/citra/config
mkdir -p ./data-backup/citra/local
cp -R $HOME/.config/citra-emu ./data-backup/citra/config
cp -R $HOME/.local/share/citra-emu ./data-backup/citra/local
sleep 02;
printf ${boldgreen}"*** Aperte ${boldred}'ENTER'${boldgreen} para voltar ao menu.${clear}\n" && read
exec ./scripts/citra.sh
...
sudo rm -r $HOME/.config/citra-emu
sudo rm -r $HOME/.local/share/citra-emu
mkdir -p $HOME/.config/citra-emu
mkdir -p $HOME/.local/share/citra-emu
cp -R ./data-backup/citra/config $HOME/.config
cp -R ./data-backup/citra/local $HOME/.local/share
sleep 03;
printf ${boldgreen}"*** Aperte ${boldred}'ENTER'${boldgreen} para voltar ao menu.${clear}\n" && read
exec ./scripts/citra.sh
