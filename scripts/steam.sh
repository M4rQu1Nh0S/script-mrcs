...
printf "Digite ${boldorange}1${clear} - Baixar o Steam\n\n"
# Fazer Backup
...
sleep 06;
printf ${boldgreen}"* Criando um backup do 'Steam' *${clear}\n"
rm -r ./data-backup/steam
mkdir -p ./data-backup/steam
cp -rT $HOME/.steam ./data-backup/steam
sleep 02;
printf ${boldgreen}"*** Aperte ${boldred}'ENTER'${boldgreen} para voltar ao menu.${clear}\n" && read
exec ./scripts/steam.sh
...
sudo rm -r $HOME/.steam
mkdir -p $HOME/.steam
cp -rT ./data-backup/steam $HOME/.steam
sleep 03;
printf ${boldgreen}"*** Aperte ${boldred}'ENTER'${boldgreen} para voltar ao menu.${clear}\n" && read
exec ./scripts/steam.sh
